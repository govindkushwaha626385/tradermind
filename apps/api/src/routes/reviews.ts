// ──────────────────────────────────────────────
// TradeMind — Reviews / Testimonials Routes
//
// Public:
//   GET  /reviews             → List approved reviews
//
// Authenticated:
//   POST /reviews             → Submit a review
//   GET  /reviews/mine        → Get my review
//   PUT  /reviews/mine        → Update my review
// ──────────────────────────────────────────────

import { Hono } from 'hono';
import { z } from 'zod';
import { getDatabase, reviews } from '@trademind/database';
import { eq, and, desc, sql } from 'drizzle-orm';
import { authMiddleware } from '../middleware/auth';
import { validateBody } from '../middleware/validate';

export const reviewsRouter = new Hono();

const submitReviewSchema = z.object({
  rating: z.number().int().min(1).max(5),
  headline: z.string().min(5).max(150),
  body: z.string().min(20).max(2000),
  traderType: z.string().max(100).optional(),
  displayName: z.string().max(100).optional(),
});

// ── Public: List approved reviews ────────────
reviewsRouter.get('/', async (c) => {
  const db = getDatabase();
  const limit = Math.min(Number(c.req.query('limit') ?? 20), 50);
  const featuredOnly = c.req.query('featured') === 'true';

  const conditions = [eq(reviews.isApproved, true)];
  if (featuredOnly) conditions.push(eq(reviews.isFeatured, true));

  const rows = await db
    .select({
      id: reviews.id,
      rating: reviews.rating,
      headline: reviews.headline,
      body: reviews.body,
      traderType: reviews.traderType,
      displayName: reviews.displayName,
      isFeatured: reviews.isFeatured,
      createdAt: reviews.createdAt,
    })
    .from(reviews)
    .where(and(...conditions))
    .orderBy(desc(reviews.isFeatured), desc(reviews.createdAt))
    .limit(limit);

  const [countRow] = await db
    .select({ count: sql<number>`COUNT(*)` })
    .from(reviews)
    .where(eq(reviews.isApproved, true));

  return c.json({
    success: true,
    data: rows,
    total: Number(countRow?.count ?? 0),
  });
});

// ── Authenticated: Submit review ──────────────
reviewsRouter.post('/', authMiddleware, validateBody(submitReviewSchema), async (c) => {
  const user = c.get('user');
  const body = c.get('validatedBody');
  const db = getDatabase();

  // One review per user — upsert
  const [existing] = await db
    .select({ id: reviews.id })
    .from(reviews)
    .where(eq(reviews.userId, user.id))
    .limit(1);

  if (existing) {
    const [updated] = await db
      .update(reviews)
      .set({
        rating: body.rating,
        headline: body.headline,
        body: body.body,
        traderType: body.traderType,
        displayName: body.displayName,
        isApproved: false, // re-review on edit
        updatedAt: new Date(),
      })
      .where(eq(reviews.id, existing.id))
      .returning();

    return c.json({
      success: true,
      data: updated,
      message: 'Review updated. It will appear after admin approval.',
    });
  }

  const [review] = await db
    .insert(reviews)
    .values({
      userId: user.id,
      rating: body.rating,
      headline: body.headline,
      body: body.body,
      traderType: body.traderType ?? null,
      displayName: body.displayName ?? null,
      isApproved: false,
      isFeatured: false,
    })
    .returning();

  return c.json({
    success: true,
    data: review,
    message: 'Review submitted successfully. It will appear after admin approval.',
  }, 201);
});

// ── Authenticated: My review ──────────────────
reviewsRouter.get('/mine', authMiddleware, async (c) => {
  const user = c.get('user');
  const db = getDatabase();

  const [review] = await db
    .select()
    .from(reviews)
    .where(eq(reviews.userId, user.id))
    .limit(1);

  return c.json({ success: true, data: review ?? null });
});
