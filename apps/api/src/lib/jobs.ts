 ...
 * Mirrors the former initializeWorkers() from queue.ts.
 */
export async function initializeWorkers(): Promise<void> {
  const dbUrl = process.env.DATABASE_URL ?? process.env.SUPABASE_DATABASE_URL;
  if (!dbUrl || dbUrl.includes('your-project.supabase.co')) {
    console.log('ℹ️  [Jobs] Background workers dormant (valid DATABASE_URL not yet configured)');
    return;
  }

  const { syncWorker }          = await import('../workers/sync.worker');
  const { webhookWorker }       = await import('../workers/webhook.worker');
  const { clusteringWorker }    = await import('../workers/clustering.worker');
  const { reportWorker, scheduleReports } = await import('../workers/report.worker');
  const { cleanupWorker }       = await import('../workers/cleanup.worker');
  const { tokenRefreshWorker }  = await import('../workers/token-refresh.worker');
  const { killSwitchWorker }    = await import('../workers/kill-switch.worker');
  ...
  await scheduleReports().catch((err) => {
    console.error('❌ Failed to schedule reports:', err);
  });

  // Schedule daily cleanup at midnight (using setInterval approximation)
  scheduleRecurring({
    queue: QUEUES.CLEANUP,
    jobName: 'daily-cleanup',
    payload: {},
    cronPattern: '0 0 * * *',
    jobId: 'cleanup:daily-cleanup:recurring',
    intervalMs: 24 * 60 * 60 * 1000, // 24 hours
  });

  console.log('✅ Queue workers initialized (Supabase-backed)');
}
