...
async function computeProgress(
  ...
  const trades = await db
    .select({ netPnl: journalTrades.netPnl, closedAt: journalTrades.closedAt })
    .from(journalTrades)
  ...
  switch (type) {
    ...
    case 'trade_count':
      ...
      break;
    case 'profit_factor': {
      const gw = trades.filter((t) => Number(t.netPnl ?? 0) > 0).reduce((s, t) => s + Number(t.netPnl ?? 0), 0);
      const gl = Math.abs(trades.filter((t) => Number(t.netPnl ?? 0) < 0).reduce((s, t) => s + Number(t.netPnl ?? 0), 0));
      currentValue = gl > 0 ? gw / gl : gw > 0 ? 999 : 0;
      break;
    }
    case 'avg_rr': {
      const pnls = trades.map((t) => Number(t.netPnl ?? 0));
      currentValue = pnls.length > 0 ? pnls.reduce((s, v) => s + v, 0) / pnls.length : 0;
      break;
    }
    default:
      currentValue = 0;
...
export async function GET(
  ...
  try {
    ...
    console.error('[Goals GET] Unhandled error:', err);
...
export async function POST(
  ...
  try {
    ...
    console.error('[Goals POST] Unhandled error:', err);
    const message = err instanceof Error ? err.message : 'Internal server error';
    return NextResponse.json({ success: false, error: { message } }, { status: 500 });
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error('[Goals PATCH] Unhandled error:', err);
    const message = err instanceof Error ? err.message : 'Internal server error';
    return NextResponse.json({ success: false, error: { message } }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[Goals DELETE] Unhandled error:', err);
    const message = err instanceof Error ? err.message : 'Internal server error';
    return NextResponse.json({ success: false, error: { message } }, { status: 500 });
  }
}
