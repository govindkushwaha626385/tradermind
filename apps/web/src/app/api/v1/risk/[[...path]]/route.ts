...
function evaluateKillSwitch(profile: Record<string, unknown> | null, todayPnl: number, todayTradeCount: number, consecutiveLosses: number) {
  ...
  if (dailyLimitAbs > 0 && todayPnl <= -dailyLimitAbs) return { shouldActivate: true, reason: `Daily loss limit ₹${dailyLimitAbs.toLocaleString('en-IN')} reached (Current P&L: ₹${todayPnl.toLocaleString('en-IN')})` };
  if (maxTrades > 0 && todayTradeCount >= maxTrades) return { shouldActivate: true, reason: `Maximum trade limit of ${maxTrades} trades/day reached` };
  if (maxConsec > 0 && consecutiveLosses >= maxConsec) return { shouldActivate: true, reason: `${consecutiveLosses} consecutive losses — mandatory break activated` };
...
export async function GET(
  ...
  if (action === 'status') {
    ...
    const todayTrades = await db.select({ pnl: journalTrades.netPnl }).from(journalTrades).where(and(eq(journalTrades.userId, user.id), eq(journalTrades.status, 'CLOSED'), gte(journalTrades.closedAt, start), lte(journalTrades.closedAt, end)));
    const todayPnl = todayTrades.reduce((s, t) => s + Number(t.pnl ?? 0), 0);
    const todayTradeCount = todayTrades.length;
    const recentTrades = await db.select({ pnl: journalTrades.netPnl }).from(journalTrades).where(and(eq(journalTrades.userId, user.id), eq(journalTrades.status, 'CLOSED'))).orderBy(sql`${journalTrades.closedAt} DESC`).limit(20);
    let consecutiveLosses = 0;
    for (const t of recentTrades) { if (Number(t.pnl ?? 0) < 0) consecutiveLosses++; else break; }
    ...
    if (killSwitchEnabled && !killSwitchActive) {
      const { shouldActivate, reason } = evaluateKillSwitch(profile as Record<string, unknown> | null, todayPnl, todayTradeCount, consecutiveLosses);
      if (shouldActivate && profile) {
        await db.update(riskProfiles).set({ killSwitchActive: true, killSwitchTriggeredAt: new Date(), killSwitchReason: reason }).where(eq(riskProfiles.userId, user.id));
        killSwitchActive = true; killSwitchReason = reason;
      }
    }

    const dailyLimitAbs = Number(profile?.dailyLossLimitAbs ?? 0);
    const pctOfDailyLimitUsed = dailyLimitAbs > 0 && todayPnl < 0 ? Math.min(100, Math.round((Math.abs(todayPnl) / dailyLimitAbs) * 100)) : 0;
    ...
    const maxConsec = Number(profile?.maxConsecutiveLosses ?? 0);
    if (maxConsec > 0 && consecutiveLosses > 0) warnings.push(`⚠️ ${consecutiveLosses} consecutive loss${consecutiveLosses > 1 ? 'es' : ''} — ${maxConsec - consecutiveLosses} remaining before mandatory break`);

    return ok({ profile: profile ?? null, todayPnl, todayTradeCount, consecutiveLosses, killSwitchActive, warnings, pctOfDailyLimitUsed, canTrade: !killSwitchActive, reason: killSwitchActive ? killSwitchReason : undefined });
  }
...
export async function POST(
  ...
  if (action === 'kill-switch') {
    ...
    if (sub === 'reset') {
      ...
      if (profile.killSwitchResetMode === 'admin') return apiError('Kill switch can only be reset by an administrator for this account', 403);
      await db.update(riskProfiles).set({ killSwitchActive: false, killSwitchTriggeredAt: null, killSwitchReason: null }).where(eq(riskProfiles.userId, user.id));
    ...
    const existing = await db.select({ id: riskProfiles.id }).from(riskProfiles).where(eq(riskProfiles.userId, user.id)).limit(1);
    if (existing.length === 0) {
      await db.insert(riskProfiles).values({ userId: user.id, killSwitchEnabled: true, killSwitchActive: true, killSwitchTriggeredAt: new Date(), killSwitchReason: reason });
    } else {
      await db.update(riskProfiles).set({ killSwitchActive: true, killSwitchTriggeredAt: new Date(), killSwitchReason: reason }).where(eq(riskProfiles.userId, user.id));
    }
