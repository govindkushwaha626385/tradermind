...
async function computeStrategyLiveStats(userId: string, strategyId: string) {
  ...
  const trades = await db
    .select({ netPnl: journalTrades.netPnl, rMultiple: journalTrades.rMultiple })
    .from(journalTrades)
    .where(and(
  ...
  return {
    ...
    avgRMultiple: rMultipleCount > 0 ? Number((rMultipleSum / rMultipleCount).toFixed(2)) : null,
  };
}
...
export async function GET(
  ...
  try {
    ...
    if (id && action === 'performance') {
      const [strategy] = await db.select().from(tradingStrategies)
        .where(and(eq(tradingStrategies.id, id), eq(tradingStrategies.userId, user.id))).limit(1);
      if (!strategy) return notFound('Strategy not found');
      const trades = await db
        .select({
          id: journalTrades.id, tradingsymbol: journalTrades.tradingsymbol,
          direction: journalTrades.direction, status: journalTrades.status,
          grossPnl: journalTrades.grossPnl, totalFeesAndTaxes: journalTrades.totalFeesAndTaxes,
          netPnl: journalTrades.netPnl, rMultiple: journalTrades.rMultiple,
          holdingPeriodMinutes: journalTrades.holdingPeriodMinutes,
          openedAt: journalTrades.openedAt, closedAt: journalTrades.closedAt,
        })
        .from(journalTrades)
        .where(and(eq(journalTrades.userId, user.id), eq(journalTrades.strategyId, id)))
        .orderBy(desc(journalTrades.openedAt));

      let winCount = 0, lossCount = 0, grossPnl = 0, totalFees = 0, totalPnl = 0;
      let grossWins = 0, grossLosses = 0, bestTradePnl = 0, worstTradePnl = 0;
      let holdingMinutesSum = 0, holdingMinutesCount = 0, rMultipleSum = 0, rMultipleCount = 0;
      for (const t of trades) {
      ...
      const performance: StrategyPerformance = {
        strategyId: strategy.id, strategyName: strategy.name, totalTrades,
        winCount, lossCount, winRate, totalPnl: Number(totalPnl.toFixed(2)),
        grossPnl: Number(grossPnl.toFixed(2)), totalFees: Number(totalFees.toFixed(2)),
        avgPnlPerTrade: totalTrades > 0 ? Number((totalPnl / totalTrades).toFixed(2)) : 0,
        profitFactor: grossLosses > 0 ? Number((grossWins / grossLosses).toFixed(2)) : grossWins > 0 ? 999 : 0,
        avgRMultiple: rMultipleCount > 0 ? Number((rMultipleSum / rMultipleCount).toFixed(2)) : 0,
        bestTradePnl: Number(bestTradePnl.toFixed(2)), worstTradePnl: Number(worstTradePnl.toFixed(2)),
        avgHoldingPeriodMinutes: holdingMinutesCount > 0 ? Math.round(holdingMinutesSum / holdingMinutesCount) : 0,
        recentTrades: trades.slice(0, 10).map((t) => ({
          id: t.id, tradingsymbol: t.tradingsymbol, direction: t.direction,
          netPnl: Number(t.netPnl ?? 0), openedAt: t.openedAt,
          closedAt: t.closedAt ?? undefined, status: t.status,
        })),
      };
    ...
    const strategies = await db.select().from(tradingStrategies)
      .where(and(...conditions)).orderBy(desc(tradingStrategies.createdAt));
    if (strategies.length === 0) return ok([]);

    const tradeAggregates = await db
      .select({ strategyId: journalTrades.strategyId, netPnl: journalTrades.netPnl, rMultiple: journalTrades.rMultiple })
      .from(journalTrades)
      .where(and(
    ...
    const enriched = strategies.map((strat) => {
      ...
      return {
        ...strat, totalTrades: s.totalTrades, winCount: s.winCount, lossCount: s.lossCount,
    ...
    console.error('[Strategies GET] Unhandled error:', err);
...
export async function POST(
  ...
  try {
    ...
    console.error('[Strategies POST] Unhandled error:', err);
    const message = err instanceof Error ? err.message : 'Internal server error';
    return NextResponse.json({ success: false, error: { message } }, { status: 500 });
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const db = getDatabase();
    const [existing] = await db.select().from(tradingStrategies)
      .where(and(eq(tradingStrategies.id, id), eq(tradingStrategies.userId, user.id))).limit(1);
    if (!existing) return notFound('Strategy not found');

    const [updated] = await db.update(tradingStrategies)
      .set({ ...body, name: body.name ? body.name.trim() : existing.name, updatedAt: new Date() })
    ...
    console.error('[Strategies PUT] Unhandled error:', err);
    const message = err instanceof Error ? err.message : 'Internal server error';
    return NextResponse.json({ success: false, error: { message } }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[Strategies DELETE] Unhandled error:', err);
    const message = err instanceof Error ? err.message : 'Internal server error';
    return NextResponse.json({ success: false, error: { message } }, { status: 500 });
  }
}
