...
export async function GET(
  ...
  if (action === 'preferences') {
    ...
    const prefs = await db.select({ type: notifications.type, channel: notifications.channel, isEnabled: notifications.isEnabled })
...
export async function PATCH(
  ...
  if (action === 'onboarding') {
    const { data: body, error: bodyErr } = await parseBody(req, z.object({
  ...
  if (action === 'preferences') {
    ...
    const [existing] = await db.select().from(notifications)
      .where(and(eq(notifications.userId, user.id), eq(notifications.type, body.type), eq(notifications.channel, body.channel as string)))
      .limit(1);

    if (existing) {
      await db.update(notifications).set({ isEnabled: body.isEnabled }).where(eq(notifications.id, existing.id));
    } else {
      await db.insert(notifications).values({ userId: user.id, type: body.type, channel: body.channel, isEnabled: body.isEnabled, isDelivered: false });
    }
